...
class SequenceClassificationTask(Task):
    ...
    def fit(
        ...
        **kwargs
        ...
        # 对抗训练
        fgm = kwargs.pop("fgm", None)
        # rdrop
        compute_kl_loss = kwargs.pop("compute_kl_loss", None)
        rdrop_rate = float(kwargs.pop("rdrop_rate"))
        ...
        for epoch in range(epochs):
            ...
            for step, inputs in enumerate(tqdm(train_generator)):
                ...
                if fgm is not None:
                    fgm.attack()
                    outputs_adv = self.module(**inputs)
                    logits_adv, loss_adv = self._get_train_loss(inputs, outputs_adv, **kwargs)
                    loss_adv = self._on_backward(inputs, outputs_adv, logits_adv, loss_adv, **kwargs)
                    fgm.restore()
